...
# Simple profiler for Esp-Idf v3.1 - show differences of time between lines
...
def profiler_color_print(message, profiler_color, line_color):
    """ Print a message to stderr with colored highlighting """
    if len(line_color) < 6:
        line_color = ""
    sys.stderr.write("%s%s%s" % (profiler_color, message,  ANSI_NORMAL + line_color))

def profiler_green_print(message, line_color):
    profiler_color_print(message, ANSI_GREEN, line_color)

def profiler_yellow_print(message, line_color):
    profiler_color_print(message, ANSI_YELLOW, line_color)

def profiler_red_print(message, line_color):
    profiler_color_print(message, ANSI_RED, line_color)
...
class Monitor(object):
    ...
    ### Begin change - Profiler - Joao Lopes
    def profile_write_line(self, line):
        # Init variables
        profiler_reading_micros = False
        profiler_reading_color = True
        profiler_line_color = ""
                ...
                if b == '(' and profiler_micros_val == 0:
                    ...
                    if profiler_micros_val > 0 and self.profiler_ult_micros > 0:
                        ...
                        if dif >= self.PROFILER_MIN_MICROS_RED:
                            profiler_red_print ("P!(" + strDif +  ")", profiler_line_color)
                        elif dif >= self.PROFILER_MIN_MICROS_YELLOW:
                            profiler_yellow_print ("P(" + strDif +  ")", profiler_line_color)
                        else:
                            profiler_green_print ("P(" + strDif +  ")", profiler_line_color)
        ...
        for line in sp:
            if line != b"":
                if self._output_enabled and (self._force_line_print or self._line_matcher.match(line)):
                    ...
                    self.profile_write_line(line + b'\n')
                    ### End change - Profiler - Joao Lopes
...
def main():
    ...
    ### Begin change - Profiler - Joao Lopes
    red_print('--- Profiler version monitor');
    ### End change - Profiler - Joao Lopes
