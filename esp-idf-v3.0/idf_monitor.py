...
def color_write(message, color, line_color):
    """ Print a message to stderr with colored highlighting """
    # sys.stdout.write("%s%s%s" % (color, message,  ANSI_NORMAL))
    if len(line_color) < 6:
        line_color = ""
    sys.stdout.write("%s%s%s" % (color, message,  ANSI_NORMAL + line_color))

def green_write(message, line_color):
    color_write(message, ANSI_GREEN, line_color)

def yellow_write(message, line_color):
    color_write(message, ANSI_YELLOW, line_color)
